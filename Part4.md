...
In order to allow you to play around cross-domain performance, try the same exercise on the [LIAR dataset](https://www.cs.ucsb.edu/~william/data/liar_dataset.zip), where you know the labels, and can thus immediately calculate the performance. You are expected to directly evaluate the model you trained on the FakeNewsCorpus. In other words, you do not need to retrain the model on the LIAR dataset.
